...
class Day(Advent):
    ...
    def split(self, data, l):
        """split data into splices of len items"""
        return [data[i:i+l] for i in range(0, len(data), l)]

    def solve1(self):
        result = (999999999999999, 0)
        layers = self.split(self.data, self.height * self.width)
        for layer in layers:
            c = layer.count(0)
            if c < result[0]:
    ...
    def solve2(self):
        ...
        layers = self.split(self.data, self.height * self.width)
        for layer in layers:
            for r, row in enumerate(self.split(layer, self.width)):
                for c, val in enumerate(row):
                    if result[(r, c)] is None and val != 2:
                        result[(r, c)] = val
        p = {0: ' ', 1: '#'}
        for r in range(self.height):
            for c in range(self.width):
                print(p[result[(r, c)]], sep='', end='')
            print(' ')
