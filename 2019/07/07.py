...
- Start the software for amplifier B. Provide it
the phase setting (1) and then whatever output signal was produced from
...
class Day(Advent):
    ...
    def solve1(self):
        result = (0, None)
        for phases in permutations(list(range(0, 4 + 1))):
            queues = []
            for phase in phases:
                q = Queue()
                q.put(phase)
                queues.append(q)
            queues.append(Queue())  # final output queue
            queues[0].put(0)  # first input value
            threads = []
            for i, phase in enumerate(phases):
                name = chr(ord('A') + i)
                t = Intcode(self.data, queues[i], queues[i+1], name=name)
                threads.append(t)
                t.start()
            threads[-1].join()
            value = queues[-1].get()
    ...
    def solve2(self):
        result = (0, None)
        for phases in permutations(list(range(5, 9 + 1))):
            queues = []
            for phase in phases:
                q = Queue()
                q.put(phase)
            ...
            for i, phase in enumerate(phases):
                ...
                try:
                    ...
                    output = queues[0]
                t = Intcode(self.data, input_, output, name=name)
                threads.append(t)
                t.start()
            threads[-1].join()
            value = threads[-1].last_output
            if value > result[0]:
                result = (value, phases)
        return str(result[0])
