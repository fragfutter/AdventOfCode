...
class Day12(Advent):
    ...
    def prepare(self):
        super(Day12, self).prepare()
        result = {}
        for line in self.data:
            node, neighbours = line.split(' <-> ')
            neighbours = list(map(int, neighbours.split(', ')))
            result[int(node)] = neighbours
        self.data = result

    def groupmembers(self, start):
        queue = [start, ]
        result = []
        while len(queue) > 0:
            node = queue.pop()
            ...
            for n in self.data[node]:
                if n in result or n in queue:
                    continue
                queue.append(n)
        return result

    def solve1(self):
        return len(self.groupmembers(0))
    ...
    def solve2(self):
        ...
        for node in self.data.keys():
            if node in seen:
                continue
            members = self.groupmembers(node)
            seen.extend(members)
            groups += 1
        return groups
