...
from advent.util import duplicates
...
class Day04(Advent):
    ...
    def solve1(self):
        ...
        for row in self.data:
            if not duplicates(row, values=False):
                result += 1
        return result
        # return sum([not duplicates(row, values=False) for row in self.data])

    def solve2(self):
        result = 0
        for row in self.data:
            row = [''.join(sorted(word)) for word in row]
            if not duplicates(row, values=False):
                result += 1
        return result
