...
from advent.turtle import Turtle, N
...
current node is clean, it becomes infected. Otherwise, it becomes cleaned. (This
is done after the node is considered for the purposes of changing direction.)
...
class Day22(Advent):
    ...
    def dump(self, data):
        ...
        for y in range(ymax, ymin - 1, -1):
            for x in range(xmin, xmax + 1):
                print(t[data[(x, y)]], end='')  # noqa
            print()

    def prepare(self):
        super(Day22, self).prepare()
    ...
    def solve1(self):
        result = 0
        p = int(self.size / 2)
        turtle = Turtle(position=(p, p), direction=N)
        data = deepcopy(self.data)
        for i in range(10000):
            val = data[turtle.position]
            if val > 0:
                turtle.turn_right()
                data[turtle.position] = 0
            else:
                turtle.turn_left()
                data[turtle.position] = 1
                result += 1
            turtle.walk()
        return result
    ...
    def solve2(self):
        ...
        for i in range(10000000):
            ...
            if val == clean:
                turtle.turn_left()
                data[turtle.position] = weak
            elif val == weak:
                data[turtle.position] = infected
                result += 1
            elif val == infected:
                turtle.turn_right()
                data[turtle.position] = flagged
            elif val == flagged:
                turtle.turn_around()
                data[turtle.position] = clean
            turtle.walk()
        return result
        pass
