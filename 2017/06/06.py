            ...
            for idx in c:
                ...
                if dist:
                    ...
                    if state[idx] == val:
                        ...
                        dist = True
            seen.append(state)
            if seen.index(state) < len(seen) - 1:
                break
        return seen

    def solve1(self):
        return len(self.solver()) - 1  # initial state requires no step

    def solve2(self):
        data = self.solver()
        return len(data) - data.index(data[-1]) - 1  # start of loop requires no step
