...
class Day24(Advent):
    ...
    def conversion(cls, line):
        return tuple(map(int, line.split('/')))  # noqa

    def prepare(self):
        super(Day24, self).prepare()
    ...
    def solver(self):
        ...
        for b in bridge.recurse():
            if b.strength > s1.strength:
                s1 = b
            if (b.length > s2.length) or \
                    (b.length == s2.length and b.strength > s2.strength):
                s2 = b
