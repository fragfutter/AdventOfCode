...
class Generator(object):
    ...
    def __init__(self, seed, factor, divisible=1):
        self.value = seed
        self.factor = factor
    ...
    def __next__(self):
        while True:
            ...
            if self.value % self.divisible == 0:
                break
        return self.value & 0b1111111111111111  # lower 16 bits
...
class Day15(Advent):
    ...
    def prepare(self):
        super(Day15, self).prepare()
        self.seed_a = int(self.data[0].split(' ')[-1])
        self.seed_b = int(self.data[1].split(' ')[-1])

    def solve1(self):
        a = Generator(self.seed_a, self.factor_a)
        b = Generator(self.seed_b, self.factor_b)
        result = 0
    ...
    def solve2(self):
        a = Generator(self.seed_a, self.factor_a, 4)
        b = Generator(self.seed_b, self.factor_b, 8)
        result = 0
