...
class Day07(Advent):
    ...
    def prepare(self):
        super(Day07, self).prepare()
        pat = re.compile('(.*) \((\d+)\)(?: -> (.*))?$')
        nodes = {}
        edges = {}
        for line in self.data:
            node, weight, neighbours = pat.match(line).groups()
            nodes[node] = int(weight)
            if neighbours:
                edges[node] = neighbours.split(', ')
        for node in nodes:
            # insert empty neighbours for leaves
    ...
    def solve1(self):
        a = list(self.nodes.keys())  # list of all nodes
        # remove all neighbours, they have a parent (node)
    ...
    def totalweight(self, node):
        ...
        for child in self.edges[node]:
            result += self.totalweight(child)
        return result

    def solve2(self):
        parent = self.solve1()
        self.totalweight(parent)
        wanted = 0
        while True:
            weights = defaultdict(list)
            for child in self.edges[parent]:
                weights[self.totalweight(child)].append(child)
            if len(weights) == 1:
                # all children have the same weight, adjust parent
                print('off balanced: %s' % parent)
                print('wanted: %s' % wanted)
                print('having: %s' % self.totalweight(parent))
                return self.nodes[parent] - self.totalweight(parent) + wanted
