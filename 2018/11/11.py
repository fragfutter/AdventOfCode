...
class Day(Advent):
    ...
    def prepare(self):
        ...
        self.serial = self.data
        data = np.empty((self.size, self.size), dtype=int)
        for x in range(self.size):
            for y in range(self.size):
                data[x][y]= self.power(x, y)
        self.data = data
    ...
    def _solve1(self):
        ...
        best = -100000000
        point = None
        for x in range(0, self.size - square + 1):
            for y in range(0, self.size - square + 1):
                value = sum([
                    sum(d[x + j][y:y+square])
    ...
    def _solve2(self):
        ...
        for x in range(0, self.size):
            ...
            for y in range(0, self.size):
                for square in range(0, self.size - max(x, y)):
                    value = sum([
                        sum(d[x + j][y:y+square])
                        for j in range(square)
                    ])
    ...
    def solve1(self):
        ...
        for x in range(0, self.size - square + 1):
            for y in range(0, self.size - square + 1):
                value = d[x:x+square, y:y+square].sum()
                if value > best:
                    point = (x, y)
                    best = value
        return '%s,%s' % point
    ...
    def solve2(self):
        ...
        for x in range(0, self.size + 1):
            ...
            for y in range(0, self.size + 1):
                for square in range(self.size - max(x, y)):
                    value = d[x:x+square, y:y+square].sum()
                    if value > best:
                        point = (x, y, square)
                        best = value
