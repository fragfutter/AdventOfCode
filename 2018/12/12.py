...
class Day(Advent):
    ...
    def prepare(self):
        ...
        initial = self.data.pop(0)
        _, initial = initial.split(': ')
        self.state = [x == '#' for x in initial]
        self.data.pop(0)
    ...
    def _mutate(self, state):
        # ensure that start has two falses
        while state[:2] != [False, False]:
            state.insert(0, False)
        # ensure that end has two falses
        while state[-2:] != [False, False]:
            state.append(False)
        result = [False, False]
        for i in range(0, len(state) - 4):
            key = tuple(state[i:i+5])
            value = self.rules[key]
            log.debug('%s => %s', key, value)
            result.append(value)
        # maybe trim leading and trailing falses
        return result
    ...
    def slicer(self, state, length=5):
        # add leading
        state = state[:]
        ...
        iters = []
        for i in range(length):
            iters.insert(0, iter(state))
            for j in iters:
                next(j)
        for key in zip_longest(*iters, fillvalue=False):
            yield key

    def mutate(self, state, length=5):
        result = []
        for key in self.slicer(state, length=length):
            result.append(self.rules[key])
        # trim end
        while not result[-1]:
            result.pop()
        # trim start
    ...
    def solve1(self):
        return self._solver(20)

    def solve2(self):
        return self._solver(50000000000)
