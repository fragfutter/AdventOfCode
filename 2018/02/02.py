...
class Day(Advent):
    ...
    def charcount(self, value, wanted):
        """check if value contains any ascii character wanted times"""
        for c in string.ascii_lowercase:
            if value.count(c) == wanted:
                return True
        return False
    ...
    def solve1(self):
        ...
        for value in self.data:
            if self.charcount(value, 2):
                two += 1
            if self.charcount(value, 3):
                three += 1
        return two * three

    def diff(self, a, b):
        """return number of character that differs and characters that are
        the same
