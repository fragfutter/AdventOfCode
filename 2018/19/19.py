...
class Cpu(object):
    ...
    def regdump(self):
        t = { 0: 'a', 1: 'b', 2: 'c', 3: 'd', 4: 'e', 5: 'f'}
        log.debug(' '.join(['%s=%s' % (t[i], v) for i, v in enumerate(self.registers)]))
...
class Day(Advent):
    ...
    def runner(self, regs):
        ...
        ipreg = self.ipreg
        loop = 0
        while ip >= 0 and ip < len(self.code):
            loop += 1
            if loop > 200000:
                break
    ...
    def solve1(self):
        return self.runner([0, 0, 0, 0, 0, 0])

    def solve2(self):
        return None
        return self.runner([1, 0, 0, 0, 0, 0])
