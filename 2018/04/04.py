...
from advent.util import histogram
...
class Day(Advent):
    ...
    def conversion(cls, line):
        ...
        """
        date = line[1:len('1518-11-05 00:03')+1]
        date = datetime.strptime(date, '%Y-%m-%d %H:%M')
        action = line[len('[1518-11-05 00:03]')+1:]
        if '#' in action:
            action = int(action.split(' ')[1][1:])
        elif 'up' in action:
            action = UP
            assert(date.time().hour == 0)
        elif 'asleep' in action:
            action = SLEEP
            assert(date.time().hour == 0)
        else:
            assert('invalid line %s' % line)
        return (date, action)

    def prepare(self):
        """create dictionary of the minutes a guard is sleeping
        guards only sleep on the 00 hour, the dict is
        guardid => [minute1day1, minute2day1, minute1day2, ..]
        """
        super(Day, self).prepare()
        a, b = sorted(self.data)[0]
        result = defaultdict(lambda: [])
        guard = None
        for date, action in sorted(self.data):
            print(date, action)
            if action not in (SLEEP, UP):
                guard = action
            elif action == SLEEP:
                assert(date.time().hour == 0)
                ...
                assert(date.time().hour == 0)
                end = date.time().minute
                for i in range(start, end):
                    result[guard].append(i)
        self.data = result

    def solve1(self):
        guard, minutes = sorted(
            self.data.items(),
            key=lambda x: len(x[1]),
            reverse=True)[0]
        hist = histogram(minutes)
        minute = sorted(hist.items(), key=lambda x: x[1], reverse=True)[0][0]
        return guard * minute

    def solve2(self):
        result = (None, 0, 0)  # guard, minute, count
        for guard, minutes in self.data.items():
            hist = histogram(minutes)
            m, c = sorted(hist.items(), key=lambda x: x[1], reverse=True)[0]
            if c > result[2]:
                result = (guard, m, c)
