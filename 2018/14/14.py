...
class Day(Advent):
    ...
    def solve1(self):
        ...
        while len(r) < (self.data + 10):
            ...
            e2 += r[e2] + 1
            e2 %= len(r)
        x = r[self.data:self.data+10]
        return '%010d' % sum([10**i * j for (i, j) in enumerate(reversed(x))])

    def solve2(self):
        search = deque(digits(self.data))
        have = deque([], len(search))  # fixed length deque, no manual pop req.
        r = [3, 7]
        e1 = 0
        e2 = 1
        while True:
