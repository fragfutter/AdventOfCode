...
class Unit(object):
    pat = re.compile(''.join([
        r"^(\d+) units each ",
        r"with\s(\d+) hit points ",
        r"(?:\((.*?)\) )?",
        r"with an attack that does (\d+) (.*?) damage ",
        r"at initiative (\d+)"]))
    ...
    def __init__(self, line, id_):
        ...
        if m.group(3):
            for skill in m.group(3).split('; '):
                t, v = skill.split(' to ')
                if t == 'weak':
                    self.weak.extend(v.split(', '))
                else:
                    self.immune.extend(v.split(', '))
        self.power = int(m.group(4))
        self.weapon = m.group(5)
    ...
    @property
    def priority(self):
        return -self.effective, -self.initiative

    def __str__(self):
        result = '%d: ' % self.id_
        result += '%d units each with %d hit points' % (self.units, self.hitpoints)  # noqa
        result += '('
        if self.weak:
            result += 'weak to %s' % ','.join(self.weak)
        if self.weak and self.immune:
            result += '; '
        if self.immune:
            result += 'immune to %s' % ','.join(self.immune)
        result += ') '
        result += "with an attack that does %d %s damage " % (self.power, self.weapon)  # noqa
        result += "at initiative %d" % self.initiative
        return result

    def choose(self, enemies):
        if not enemies:
            return None
        targets = [
            (self.damage(enemy), enemy.effective, enemy.initiative, enemy)
    ...
    def damage(self, other):
        ...
        if self.weapon in other.immune:
            return 0
        if self.weapon in other.weak:
            return result * 2
        return result

    def attack(self, other):
        kills = min(
            self.damage(other) // other.hitpoints,
            other.units
        )
        log.debug("\n%s vs \n%s", self, other)
        log.debug('%d attacks %d killing %d units',
                  self.id_, other.id_, kills)
        other.units -= kills
        if other.units == 0:
            other.dead = True
        return kills
...
class Day(Advent):
    ...
    def prepare(self):
        ...
        immune = []
        infect = []
        for id_, line in enumerate(self.data):
            if not line:
                continue
            ...
            if line.startswith('Infection'):
                target = infect
                continue
            target.append(Unit(line, id_))
        self.immune = immune
        self.infect = infect

    def targetselection(self, attacker, targets):
        targets = targets[:]
        for a in sorted(attacker, key=attrgetter('priority')):
            t = a.choose(targets)
            if t:
                targets.remove(t)
                yield a, t

    def battle(self, boost=0):
        has_kill = True
        immune = deepcopy(self.immune)
        infect = deepcopy(self.infect)
        for i in immune:
            i.power += boost
        while immune and infect and has_kill:
            has_kill = False
            pairs = list(self.targetselection(immune, infect))
            pairs.extend(list(self.targetselection(infect, immune)))
            pairs = sorted(
                pairs,
            ...
            for a, b in pairs:
                if a.dead:
                    log.debug('%d is already dead', a.id_)
                    continue
                if a.attack(b):
                    has_kill = True  # break if nothing changes
            # wipeout
            immune = [i for i in immune if not i.dead]
            infect = [i for i in infect if not i.dead]
        return (
            sum([i.units for i in immune]),
            sum([i.units for i in infect]),
        )

    def solve1(self):
        return max(self.battle())

    def solve2(self):
        for boost in count(1):
            a, b = self.battle(boost)
            if b:
                continue
            return a
